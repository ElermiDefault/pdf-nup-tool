...
BUILD_DIR="$ROOT_DIR/build/native-dmg"
APP_NAME="PDF N-up Tool"
APP_PACKAGE_NAME="pdf-nup-tool-${VERSION}-macos-native"
APP_PACKAGE_DIR="$DIST_DIR/$APP_PACKAGE_NAME"
APP_PATH="$APP_PACKAGE_DIR/${APP_NAME}.app"
DMG_PACKAGE_NAME="pdf-nup-tool-${VERSION}-macos-native"
...
"$ROOT_DIR/scripts/build_native_app.sh" "$VERSION" >/dev/null
...
Open \`PDF N-up Tool.app\` from Applications. This version uses a native
macOS window with an embedded WebView instead of opening your browser.
